...
export default function Home() {
  ...
  // Sync the html data-theme attribute on first mount
  useEffect(() => {
    document.documentElement.dataset.theme = theme;
    document.documentElement.classList.toggle('dark', theme === 'dark');
  }, [theme]);

  // Only the active step is shown in visualization (after Start clicked)
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-base font-bold tracking-tight">
            CodeVision <span style={{ color: 'var(--cv-accent)' }}>AI</span>
          </span>
          <span
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* LEFT: editor (top) + xterm console (bottom) — ~40% */}
        <div
          ...
          >
            ...
            <div className="flex-1 overflow-hidden p-1.5" style={{ background: 'var(--cv-bg)' }}>
              <XTermConsole
                ...
                registerHandle={registerTerminal}
              />
            </div>
            {error && (
              <div
        ...
        {/* RIGHT: visualization (~60%) — stack | heap with arrows */}
        <div className="flex flex-col flex-1 overflow-hidden">
          <PanelTab label="Stack ◂ ─ ▸ Heap" dotColor="var(--cv-active-border)" />
          <VisualizationPanel
            state={currentState}
            status={status}
      ...
      >
        ...
        >
          {steps.length > 0
            ? `${hasStarted ? viewIndex + 1 : 0} / ${steps.length}`
            : '0 / 0'}
        </span>
      </div>
    </div>
